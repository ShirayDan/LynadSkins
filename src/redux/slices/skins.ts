...
export const fetchAllSkins = createAsyncThunk(
	'skins/fetchAllSkins',
	async ([userId, filters]: Array<string>) => {
...
const skinsSlice = createSlice({
	...
	extraReducers: (builder) => {
		builder
			...
			.addCase(
				fetchAllSkins.fulfilled,
				(state, action: PayloadAction<Array<TSkin>>) => {
					state.status = 'loaded'
					state.items = action.payload
				}
			)
		...
		builder
			...
			.addCase(createSkin.fulfilled, (state, action: any) => {
				state.status = 'loaded'
				state.items = action.payload
			})
